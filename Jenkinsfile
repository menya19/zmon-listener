...
podTemplate(label:label,
    containers: [
        ...
        containerTemplate(name: 'helm', image: 'lachlanevenson/k8s-helmv2.9.1', ttyEnabled: true, command: 'cat')
    ],
    volumes: [
    ...
    node(label) {
        ...
        stage('LISTENER INGRESS'){
          container('kubectl') {
            withEnv(['KUBECONFIG=/var/config/zmon/kube-config-zmon.yml']) {
              yaml.update file: 'ingress.yaml', update: ['.metadata.name': "${TENANT}-ingress", '.metadata.namespace': "${TENANT}", '.spec.rules[0].host': "${TENANT}.zmon.cloud", '.spec.rules[0].http.paths[0].backend.serviceName': "listener-${TENANT}"]
              sh "cat ingress.yaml"
